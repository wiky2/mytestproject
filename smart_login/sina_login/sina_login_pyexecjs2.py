...
def get_runntime(path):
    ...
    """
    # sys.path.append('/Users/mateseries/Downloads/phantomjs-2.1.1-macosx/bin/phantomjs')
    # phantom = execjs.get('/Users/mateseries/Downloads/phantomjs-2.1.1-macosx/bin/phantomjs')  # 这里必须为phantomjs设置环境变量，否则可以写phantomjs的具体路径
    # phantom = execjs.get('phantomjs')  # 这里必须为phantomjs设置环境变量，否则可以写phantomjs的具体路径
    # with open(path, 'r') as f:
    #     source = f.read()
    f = open("./sinalogin.js", 'r')
    jsscript = f.read()
...
if __name__ == '__main__':
    ...
    session = get_session()
    print session
    runntime = get_runntime('./sinalogin.js')#当前目录
    print runntime
