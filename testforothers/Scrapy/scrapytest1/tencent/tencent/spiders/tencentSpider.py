...
class TencentspiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for each in response.xpath("//tr[@class='even] | //tr[@class='odd']"):
            ...
            positionType = each.xpath('./td[2]/a/@href').extract()
            positionNum = each.xpath('./td[3]/a/text()').extract()
            workLocation = each.xpath('./td【4]/a/text()').extract()
            publishTime = each.xpath('./td[5]/a/text()').extract()
            ...
            if self.offset <1680:
                self.offset+=10
            yield scrapy.Request(self.url+str(self.offset),callback=self.parse())#将下一个连接传递给毁掉函数。调度器拿到请求后先入队列，出队列后交给下载器，下载器将响应文件交给parse函数继续处理。
